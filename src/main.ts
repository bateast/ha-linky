...
import { formatAsStatistics, incrementSums } from './format.js';
import { HomeAssistantClient } from './ha.js';
import { getMeterHistory } from './history.js';
import { runFill } from './index.js';
...
export async function main() {
  ...
  async function sync(config: MeterConfig) {
    ...
    const fillDay = dayjs().subtract(config.fill ? config.fill : 0, 'day');

    const isSyncingNeeded =
      (dayjs(lastStatistic.start).isBefore(dayjs().subtract(2, 'days')) && dayjs().hour() >= 6) || config.fill;
    if (!isSyncingNeeded) {
    ...
    const energyData = formatAsStatistics(newData);
    await haClient.saveStatistics({
      prm: config.prm,
      name: config.name,
      isProduction: config.production,
